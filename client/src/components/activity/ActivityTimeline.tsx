...
export function ActivityTimeline({ 
  ...
  const toggleActivity = (activity: Activity) => {
    const activityId = `${activity.title}-${activity.type}-${activity.start_location}-${activity.end_location}`;
    setExpandedActivity(expandedActivity === activityId ? null : activityId);
  };
  
  const getActivityId = (activity: Activity) => {
    return `${activity.title}-${activity.type}-${activity.start_location}-${activity.end_location}`;
  };
